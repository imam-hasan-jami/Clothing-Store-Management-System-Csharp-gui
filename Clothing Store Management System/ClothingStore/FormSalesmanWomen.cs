...
namespace LogInForm
{
    public partial class FormSalesmanWomen : Form
    {
        private DataAccess da { get; set; }

        public FormSalesmanWomen()
        {
            InitializeComponent();
        ...
        private void PopulateGridView(string sql)
        {
            ...
            this.dgvWomensProduct.AutoGenerateColumns = false;
            this.dgvWomensProduct.DataSource = ds.Tables[0];
        }
        ...
        private void FormCustomerWomen_Load(object sender, EventArgs e)
        {
            //grid view
        ...
        {
            this.Hide();
            FormSalesmanKids fck = new FormSalesmanKids();
            fck.Show();
        }
        ...
        private void cmbOrigin_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                string sql = "SELECT * FROM WomensProduct where Origin = '" + this.cmbOrigin.Text + "';";
                this.PopulateGridView(sql);
            }
            else
            {
        ...
        private void cmbSize_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                string sql = "SELECT * FROM WomensProduct where Size = '" + this.cmbSize.Text + "';";
                this.PopulateGridView(sql);
            }
            else
            {
        ...
        private void cmbPrice_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                string sql = "SELECT * FROM WomensProduct WHERE Price BETWEEN 0 AND 499";
                this.PopulateGridView(sql);
            }
            else if (this.cmbPrice.Text == "500 to 999")
            {
                string sql = "SELECT * FROM WomensProduct WHERE Price BETWEEN 500 AND 999";
                this.PopulateGridView(sql);
            }
            else if (this.cmbPrice.Text == "1000 to 1499")
            {
                string sql = "SELECT * FROM WomensProduct WHERE Price BETWEEN 1000 AND 1499";
                this.PopulateGridView(sql);
            }
            else if (this.cmbPrice.Text == "over 1500")
            {
                string sql = "SELECT * FROM WomensProduct WHERE Price > 1500";
                this.PopulateGridView(sql);
            }
            else
            {
